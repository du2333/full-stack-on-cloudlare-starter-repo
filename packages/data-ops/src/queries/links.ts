...
import { and, count, desc, eq, gt, lt, max, sql } from "drizzle-orm";
...
export async function getLinks(accoundid: string, createdBefore?: string) {
  const db = getDb();

  const conditions = [eq(links.accountId, accoundid)];

  if (createdBefore) {
    conditions.push(lt(links.created, createdBefore));
  }
